...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  await validateUserRole(request, [UserRole.librarian]);

  const librarian = await getLibrarianById(userId);

  return json({
    hasResetPassword: librarian!.hasResetPassword,
  });
};
      ...
      <AppShell.Section component={ScrollArea} grow mb="sm" mt={36} offsetScrollbars p="md">
        <div className="flex flex-col gap-1">
          ...
          <SidebarNav.Group label="Manage" leftIcon={<FolderKanbanIcon size={14} />}>
            <SidebarNav.Link end label="Books" to="/librarian/books" />
            <SidebarNav.Link end label="Rented Books" to="/librarian/rented-books" />
            <SidebarNav.Link end label="Purchased Books" to="/librarian/purchased-books" />
          </SidebarNav.Group>
        </div>
      </AppShell.Section>
